...
export function useHistory({
  ...
  // Marks the last non-reverted turn as reverted
  const undo = useCallback(() => {
    setHistory((prevHistory) => {
      ...
      if (
        ...
        if (typeof lastText === 'string' && lastText.trim().startsWith('/undo')) {
          historyCopy.pop();
        }
      }
  ...
  return useMemo(
    ...
    [history, addItem, updateItem, clearItems, undo, loadHistory],
  );
}
